    ...
    def load_outer_memory(cls, traderData):
        ...
        if traderData.startswith(cls.OUTER_PREFIX):
            try:
                ...
                if isinstance(decoded, dict): return {str(k): (v if isinstance(v, str) else "") for k, v in decoded.items()}
            except Exception:
                return {}
        try:
...
class RestOfFollowTrader:
    ...
    def trade_flow_product(
        ...
        if bid_vol <= 0 or ask_vol <= 0: return []
...
class TranslatorTrader:
    ...
    def product_score(self, product):
        ...
        if len(hist) < max(self.LOOKBACK, self.VOL_LOOKBACK) + 1: return None
        now = hist[-1]
    ...
    def build_targets(self):
        ...
        if len(scores) < len(self.PRODUCTS): return targets
        ...
        if best_score - worst_score < self.MIN_SCORE_GAP: return targets
...
class RobotTrader:
    ...
    def best_bid(self, depth):
        if not depth.buy_orders: return None
        return max(depth.buy_orders.keys())
    def best_ask(self, depth):
        if not depth.sell_orders: return None
    ...
    def run(self, state):
        ...
        for p in self.ALL_ROBOTS:
            result[p] = []
        for p in self.ALL_ROBOTS:
            if p not in state.order_depths: return result, 0, json.dumps(data)
        mids = {}
        for p in self.ALL_ROBOTS:
            m = self.mid(state.order_depths[p])
            if m is None: return result, 0, json.dumps(data)
            mids[p] = m
        ...
        if len(dev_hist[self.DISHES]) < self.min_history: return result, 0, json.dumps(data)
        candidates = []
...
class PebblesTrader:
    ...
    def run(self, state):
        ...
        for product in self.PRODUCTS:
            if product not in state.order_depths: return result, conversions, jsonpickle.encode(memory)
        mids = {}
        for product in self.PRODUCTS:
            mid = self.get_mid(state.order_depths[product])
            if mid is None: return result, conversions, jsonpickle.encode(memory)
            mids[product] = mid
    ...
    def update_side_scanner(self, data, mids):
        ...
        if min_hist_len < self.SCAN_MIN_HISTORY: return targets, False
        z_scores = {}
        for product in self.SIDE_SCAN_PRODUCTS:
    ...
    def side_scanner_size(cls, z_spread):
        if z_spread < 2.90: return 2
        if z_spread < 3.70: return 3
    ...
    def scanner_size(z_spread):
        if z_spread < 2.75: return 3
        if z_spread < 3.55: return 6
        return 10
    def take_toward_target(self, product, od, current_pos, target_pos, max_size):
        orders = []
        diff = target_pos - current_pos
        if diff > 0:
            if not od.sell_orders: return orders
            ...
            if not od.buy_orders: return orders
            ...
            if qty > 0:
                orders.append(Order(product, best_bid, -qty))
        return orders
    def passive_toward_target(self, product, od, current_pos, target_pos, max_size, improve):
        orders = []
        diff = target_pos - current_pos
        if not od.buy_orders or not od.sell_orders: return orders
        best_bid = max(od.buy_orders.keys())
        best_ask = min(od.sell_orders.keys())
        if diff > 0:
            qty = min(diff, max_size, self.POSITION_LIMIT - current_pos)
            if qty <= 0: return orders
...
class GalaxyTrader:
    ...
    def combined_score(self, product):
        ...
        if fast is None and slow is None: return None
        if slow is None: return fast
    ...
    def rings_target(self):
        ...
        if score is None: return self.RINGS_BASE_SHORT
        if score >= self.RINGS_FLAT_SCORE: return self.RINGS_FLAT
        if score >= self.RINGS_REDUCE_SHORT_SCORE: return self.RINGS_REDUCED_SHORT
        if score <= self.RINGS_STRONG_SHORT_SCORE: return self.RINGS_STRONG_SHORT
    ...
    def product_trade_cap(self, product):
        if product == self.RINGS: return self.MAX_TRADE_SIZE
...
class PanelTrader:
    ...
    def basket_health_score(self):
        ...
        if len(hist) < max(self.BASKET_LOOKBACK, self.REGIME_VOL_LOOKBACK) + 1: return 0.0
    ...
    def leg_support_score(
        ...
        hist = self.mid_history[product]
        if len(hist) < max(self.LEG_LOOKBACK, self.REGIME_VOL_LOOKBACK) + 1: return 0.0
    ...
    def static_scale(self):
        ...
        if score <= self.BASKET_BAD_SCORE: return 0.40
        if score <= self.BASKET_WEAK_SCORE: return 0.70
        return 1.00
    def leg_scale(self, product, intended_direction):
        score = self.leg_support_score(product, intended_direction)
        if score <= self.LEG_BAD_SCORE: return 0.40
        if score <= self.LEG_WEAK_SCORE: return 0.70
        return 1.00
...
class SleepTrader:
    ...
    def shock_score(self, product):
        hist = self.mid_history[product]
        if len(hist) < self.SHOCK_LOOKBACK + self.VOL_LOOKBACK + 1: return 0.0
        now = hist[-1]
    ...
    def rolling_z(self, product):
        hist = self.mid_history[product]
        if len(hist) < self.Z_LOOKBACK: return None
...
class SnackpackTrader:
    ...
    def get_mid_price(self, state, product):
        ...
        if bid is None or ask is None: return None
        return (bid + ask) / 2
    def add_buy_order(self, orders, product, price, quantity, current_position):
        qty = min(quantity, self.POSITION_LIMIT - current_position)
        if qty > 0:
            orders.append(Order(product, int(price), qty))
    def add_sell_order(self, orders, product, price, quantity, current_position):
        qty = min(quantity, self.POSITION_LIMIT + current_position)
        if qty > 0:
            orders.append(Order(product, int(price), -qty))
    ...
    def would_pass_pair_imbalance_filter(self, state, cfg, product, delta_qty):
        ...
        if abs(proposed) <= cfg.max_pair_imbalance: return True
        if abs(current) > cfg.max_pair_imbalance and abs(proposed) < abs(current): return True
        return False
    def filter_pair_imbalance_orders(self, state, cfg, orders_a, orders_b):
        filtered_a = [o for o in orders_a if self.would_pass_pair_imbalance_filter(state, cfg, cfg.product_a, o.quantity)]
        filtered_b = [o for o in orders_b if self.would_pass_pair_imbalance_filter(state, cfg, cfg.product_b, o.quantity)]
        return filtered_a, filtered_b
    def add_pair_imbalance_repair_orders(self, state, cfg, orders_a, orders_b):
        if not cfg.repair_imbalance: return orders_a, orders_b
        imbalance = self.pair_imbalance(state, cfg)
        if abs(imbalance) <= cfg.max_pair_imbalance: return orders_a, orders_b
        pos_a = state.position.get(cfg.product_a, 0)
        pos_b = state.position.get(cfg.product_b, 0)
        bid_a, ask_a = self.get_best_bid_ask(state, cfg.product_a)
        bid_b, ask_b = self.get_best_bid_ask(state, cfg.product_b)
        if bid_a is None or ask_a is None or bid_b is None or ask_b is None: return orders_a, orders_b
        candidates = []
        ...
        if not candidates: return orders_a, orders_b
    ...
    def close_position_if_edge_disappeared(self, orders, product, position, best_bid, best_ask, adjusted_fair, close_edge):
        if position > 0 and best_bid >= adjusted_fair - close_edge:
            self.add_sell_order(orders, product, best_bid, min(2, position), position)
        elif position < 0 and best_ask <= adjusted_fair + close_edge:
            self.add_buy_order(orders, product, best_ask, min(2, -position), position)
    ...
    def product_passive_edge(self, cfg, product):
        return cfg.passive_edge + self.PRODUCT_PASSIVE_EDGE_EXTRA.get(product, 0)
    ...
    def trade_against_fair(self, state, product, fair_value, basket_z, cfg):
        ...
        if reduce_only:
            self.close_position_if_edge_disappeared(orders, product, position, best_bid, best_ask, adjusted_fair, cfg.close_edge)
            return orders
        if buy_edge > take_edge:
            self.add_buy_order(orders, product, best_ask, cfg.take_size, position)
            return orders
        if sell_edge > take_edge:
            self.add_sell_order(orders, product, best_bid, cfg.take_size, position)
            return orders
        ...
        if passive_buy_edge > passive_edge:
            self.add_buy_order(orders, product, passive_bid, cfg.passive_size, position)
        elif passive_sell_edge > passive_edge:
            self.add_sell_order(orders, product, passive_ask, cfg.passive_size, position)
        else:
            self.close_position_if_edge_disappeared(orders, product, position, best_bid, best_ask, adjusted_fair, cfg.close_edge)
        return orders
    def trade_pair(self, state, cfg, data):
        ...
        if mid_a is None or mid_b is None: return orders_a, orders_b
        ...
        if len(history) < cfg.min_history: return orders_a, orders_b
        basket_mean = _statistics.mean(history)
        basket_std = _statistics.pstdev(history)
        if basket_std == 0: return orders_a, orders_b
        basket_z = (basket - basket_mean) / basket_std
        a_fair = basket_mean - cfg.weight * mid_b
        b_fair = (basket_mean - mid_a) / cfg.weight
        orders_a = self.trade_against_fair(state, cfg.product_a, a_fair, basket_z, cfg)
        orders_b = self.trade_against_fair(state, cfg.product_b, b_fair, basket_z, cfg)
        orders_a, orders_b = self.filter_pair_imbalance_orders(state, cfg, orders_a, orders_b)
        orders_a, orders_b = self.add_pair_imbalance_repair_orders(state, cfg, orders_a, orders_b)
