...
class ReplayBacktester:
    def __init__(self, data: HistoricalData, trader, position_limits: Dict[str, int] | None = None):
        self.data = data
        self.trader = trader
        self.position_limits = position_limits or getattr(trader, 'POSITION_LIMITS', {})
    ...
    def _record_fill(self, timestamp: int, symbol: str, price: int, quantity: int, reason: str) -> None:
        side = 'BUY' if quantity > 0 else 'SELL'
        self.positions[symbol] += quantity
        self.cash -= price * quantity
        fill = Fill(timestamp=timestamp, symbol=symbol, side=side, price=price, quantity=abs(quantity), reason=reason)
        self.fills.append(fill)
        self.own_trades[symbol].append(
            Trade(symbol=symbol, price=price, quantity=abs(quantity), buyer='SUBMISSION' if quantity > 0 else None,
                  seller='SUBMISSION' if quantity < 0 else None, timestamp=timestamp)
        )
    ...
    def _execute_aggressive(self, ts: int, order: Order, depth: OrderDepth) -> int:
        ...
        if remaining > 0:
            ...
            for ask_price, ask_vol_neg in asks:
                ...
                if clipped > 0:
                    self._record_fill(ts, order.symbol, ask_price, clipped, 'aggressive')
                    remaining -= clipped
        elif remaining < 0:
            ...
            for bid_price, bid_vol in bids:
                ...
                clipped = -self._check_limits(order.symbol, -fill_qty)
                if clipped > 0:
                    self._record_fill(ts, order.symbol, bid_price, -clipped, 'aggressive')
                    need -= clipped
            remaining = -need
        return remaining

    def _execute_passive(self, start_ts: int, end_ts: int, order: Order, remaining: int, interval_trades: Dict[str, List[Trade]]) -> None:
        if remaining == 0:
            return
        ...
        if remaining > 0:
            ...
            clipped = self._check_limits(order.symbol, fill_qty)
            if clipped > 0:
                self._record_fill(end_ts, order.symbol, order.price, clipped, 'passive')
        else:
            need = -remaining
            cap = 0
            for tr in tape:
    ...
    def _mark_to_market(self, ts: int, mids: Dict[str, float]) -> None:
        inventory_value = sum(self.positions[s] * mids.get(s, 0.0) for s in self.positions)
        total = self.cash + inventory_value
        row = {'timestamp': ts, 'cash': self.cash, 'inventory_value': inventory_value, 'total_pnl': total}
        for symbol, pos in sorted(self.positions.items()):
            row[f'pos_{symbol}'] = pos
    ...
    def run(self, max_steps: int | None = None) -> BacktestResult:
        ...
        for i, snap in enumerate(snapshots):
            next_ts = snapshots[i + 1].timestamp if i + 1 < len(snapshots) else snap.timestamp + 100
            interval_trades = self.data.get_interval_trades(snap.timestamp, next_ts)
            current_market_trades = {sym: list(trs) for sym, trs in interval_trades.items()}
            ...
            for symbol, orders in result.items():
                ...
                for order in orders:
                    if order.quantity == 0:
                        continue
                    remaining = self._execute_aggressive(snap.timestamp, order, depth)
                    self._execute_passive(snap.timestamp, next_ts, order, remaining, interval_trades)

            self._mark_to_market(snap.timestamp, snap.mids)

        equity = pd.DataFrame(self.equity_rows)
        final_pnl = float(equity['total_pnl'].iloc[-1]) if not equity.empty else 0.0
